...
public class CategoryServicesImpl implements ICategoryService {
	...
	public ResponseEntity<CategoryResponseRest> searchById(Long id) {
		...
		try {
			...
			response.setMetadata("Respuesta notOk", "-1", "Error al consultar por id");
			e.getStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<CategoryResponseRest> save(Category category) {
		...
		try {
			...
			if (categorySaved != null) {
				...
				response.setMetadata("Respuesta notOk", "-1", "Categoría no guardada.");
				return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.BAD_REQUEST);
			}

		} catch (Exception e) {
			response.setMetadata("Respuesta notOk", "-1", "Error al grabar categoría");
			e.getStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);
